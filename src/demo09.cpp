...
//
// Phong shaded texture mapping demonstration
//
#include "lib/types.h"
...
word		vscreen, enviromap, texturemap;
...
void	drawfacets ( void ) {
	...
	for ( bin = 79; bin >= 0; bin -- ) {
		...
		for ( binfacet = 0; binfacet < bintotal; binfacet ++ ) {
			...
			envirotexturetriangle ( v1, v2, v3, enviromap, texturemap, shadetable, vscreen );
		}
		bincount[bin] = 0;
	}
}
...
void	main ( void ) {
	...
	clrscr ();
	printf ( "\n" );
	printf ( "Phong shaded texture mapping demonstration\n\n" );
