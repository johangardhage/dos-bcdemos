...
//
// Gouraud shaded texture mapping demonstration
//
#include "lib/types.h"
...
void	calcvertexcolors ( void ) {
	...
	for ( loop = 0; loop < objectptr->totalvertices; loop ++ ) {
		...
		if (costheta < 0) v->color = abs(costheta * 128);
		else v->color = 1;
		if (v->color > 127) v->color = 127;
		if (v->color < 1) v->color = 1;
	}
}
...
void	drawfacets ( void ) {
	...
	for ( bin = 79; bin >= 0; bin -- ) {
		...
		for ( binfacet = 0; binfacet < bintotal; binfacet ++ ) {
			...
			gouraudtexturetriangle ( v1, v2, v3, texturemap, shadetable, vscreen );
		}
		bincount[bin] = 0;
	}
}
...
void	main ( void ) {
	...
	clrscr ();
	printf ( "\n" );
	printf ( "Gouraud shaded texture mapping demonstration\n\n" );
