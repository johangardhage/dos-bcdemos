...
//
// Phong shaded texture bump mapping demonstration
//
#include "lib/types.h"
...
char		bumpfile[]	= "assets/mask.bum";
char		shadefile[]	= "assets/mask.sht";
word		vscreen, enviromap, texturemap, bumpmap;
...
void	drawfacets ( void ) {
	...
	for ( bin = 79; bin >= 0; bin -- ) {
		...
		for ( binfacet = 0; binfacet < bintotal; binfacet ++ ) {
			...
			envirotexturebumptriangle ( v1, v2, v3, enviromap, texturemap, bumpmap, shadetable, vscreen );
		}
		bincount[bin] = 0;
	}
}
...
void	main ( void ) {
	...
	clrscr ();
	printf ( "\n" );
	printf ( "Phong shaded texture bump mapping demonstration\n\n" );
	...
	enviromap = initphongmap (128);
	texturemap = initpicture (texturefile);
	bumpmap = initpicture (bumpfile);
